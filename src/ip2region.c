...
/**
 * get the region associated with the specified ip address with the memory
 ...
 * @param   datablock
*/
IP2R_API uint_t ip2region_memory_search(ip2region_t ip2rObj, uint_t ip,
  ...
  if (ip2rObj->dbBinStr == NULL) {
    ...
    ip2rObj->dbBinStr = (char *)IP2R_MALLOC(filesize);
    if (ip2rObj->dbBinStr == NULL) {
      return 0;
    }
    ...
    ip2rObj->totalBlocks =
        (ip2rObj->lastIndexPtr - ip2rObj->firstIndexPtr) / INDEX_BLOCK_LENGTH +
        1;
  }

  l = 0;
  h = ip2rObj->totalBlocks;
  dptr = 0;
  while (l <= h) {
    m = (l + h) >> 1;
...
IP2R_API uint_t ip2region_memory_search_string(ip2region_t ip2rObj, char *ip,
                                               datablock_t datablock) {
  return ip2region_memory_search(ip2rObj, ip2long(ip), datablock);
}

/**
 * get the region associated with the specifield ip address with binary search
 * algorithm
 *
 * @param    ip2rObj
 * @param    ip
 ...
 * @return    uint_t
*/
IP2R_API uint_t ip2region_binary_search(ip2region_t ip2rObj, uint_t ip,
                                        datablock_t datablock) {
  int l, h, m, p;
  uint_t sip, eip, dptr;
  char buffer[256];
...
IP2R_API uint_t ip2region_binary_search_string(ip2region_t ip2rObj, char *ip,
                                               datablock_t datablock) {
  return ip2region_binary_search(ip2rObj, ip2long(ip), datablock);
}

/**
 * get the region associated with the specifield ip address with b-tree
 * algorithm
 *
 * @param    ip2rObj
 * @param    ip
 ...
 * @return    uint_t
*/
IP2R_API uint_t ip2region_btree_search(ip2region_t ip2rObj, uint_t ip,
                                       datablock_t datablock) {
  int i, idx;
  ...
  if (fread(buffer, indexBlockLen + INDEX_BLOCK_LENGTH, 1,
            ip2rObj->dbHandler) != 1) {
    return 0;
  }
  ...
  dptr = 0;
  l = 0;
  h = indexBlockLen / INDEX_BLOCK_LENGTH;
  while (l <= h) {
    m = ((l + h) >> 1);
...
IP2R_API uint_t ip2region_btree_search_string(ip2region_t ip2rObj, char *ip,
                                              datablock_t datablock) {
  return ip2region_btree_search(ip2rObj, ip2long(ip), datablock);
}

/**
 * get a unsinged long(4bytes) from a specifield buffer start from the
 * specifield offset
 *
 * @param    buffer
 * @param    offset
...
*/
IP2R_API uint_t long2ip(uint_t ip, char *buffer) { return 0; }
